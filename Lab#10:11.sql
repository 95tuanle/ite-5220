...
CREATE OR REPLACE PACKAGE BODY check_emp_pkg IS
    ...
    PROCEDURE chk_dept_mgr (p_empid IN employees.employee_id%TYPE, p_mgr IN employees.manager_id%TYPE) IS
        ...
        BEGIN
            select DEPARTMENTS.MANAGER_ID into v_manager_id from DEPARTMENTS join EMPLOYEES on DEPARTMENTS.DEPARTMENT_ID = EMPLOYEES.DEPARTMENT_ID where EMPLOYEE_ID = 174;
...
CREATE OR REPLACE PACKAGE BODY init_pkg IS
    PROCEDURE get_emp_sal (p_emp_id IN employees.employee_id%TYPE) IS
        ...
        BEGIN
            ...
            END IF;
        END get_emp_sal;
    BEGIN SELECT MAX(salary) INTO g_max_sal FROM employees;
END init_pkg;
...
begin
    DBMS_OUTPUT.PUT_LINE(v_days||' days is '||time_conversion_cons.c_day_to_hr*v_days||' hours or '||time_conversion_cons.c_day_to_min*v_days||' minutes or '||time_conversion_cons.c_day_to_sec*v_days||' seconds,');
    DBMS_OUTPUT.PUT_LINE(v_hours||' hours is '||time_conversion_cons.c_hr_to_day*v_hours||' days or '||time_conversion_cons.c_hr_to_min*v_hours||' minutes or '||time_conversion_cons.c_hr_to_sec*v_hours||' seconds,');
    DBMS_OUTPUT.PUT_LINE(v_minutes||' minutes is '||time_conversion_cons.c_min_to_day*v_minutes||' days or '||time_conversion_cons.c_min_to_hr*v_minutes||' hours or '||time_conversion_cons.c_min_to_sec*v_minutes||' seconds,');
    DBMS_OUTPUT.PUT_LINE(v_seconds|| ' seconds is '||time_conversion_cons.c_sec_to_day*v_seconds|| ' days or '||time_conversion_cons.c_sec_to_hr*v_seconds|| ' hours or '|| time_conversion_cons.c_sec_to_min * v_seconds|| ' minutes.');
end;
...
create or replace package cursor_state is
    cursor emp_dept_curs is select distinct FIRST_NAME, LAST_NAME, DEPARTMENT_NAME, SALARY from EMPLOYEES join DEPARTMENTS on EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID order by EMPLOYEE_ID;
    procedure open_curs;
    ...
    procedure fetch_n_rows (p_number in number) is
        ...
        begin
            ...
            if emp_dept_curs%isopen then
                for v_count in 1..p_number loop
                    DBMS_OUTPUT.PUT('Row ' || v_count || ' ');
                    fetch emp_dept_curs into v_emp;
                    exit when emp_dept_curs%notfound;
                    DBMS_OUTPUT.PUT_LINE(v_emp.first_name||' '||v_emp.last_name||' '||v_emp.department_name||' '||v_emp.salary);
                end loop;
            end if;
        end fetch_n_rows;
...
select OBJECT_NAME from ALL_OBJECTS where OBJECT_TYPE = 'PACKAGE' and OWNER = 'SYS' order by OBJECT_NAME;


CREATE OR REPLACE PROCEDURE display_proc IS
    BEGIN
        FOR i IN 1..10 LOOP DBMS_OUTPUT.PUT(i || ' ');
    END LOOP;
...
create or replace procedure display_emp_names (p_names out varchar2) is
    cursor emp_curs is select LAST_NAME from EMPLOYEES where DEPARTMENT_ID = 80 order by LAST_NAME ASC;
begin
    for v_name in emp_curs loop
...
CREATE OR REPLACE PROCEDURE list_sals IS
    ...
    CURSOR empc IS sELECT last_name, salary FROM employees ORDER BY salary;
    BEGIN
...
SELECT object_name, timestamp FROM USER_OBJECTS WHERE object_type = 'FUNCTION';
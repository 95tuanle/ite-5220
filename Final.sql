...
begin
    ...
    for i in v_first_names.first..v_first_names.LAST
        loop
...
create or replace trigger department_audit_trigger
    ...
    for each row
begin
    insert into Department_Audit (department_id, old_department_name, new_department_name)
    VALUES (:new.DEPARTMENT_ID, :old.DEPARTMENT_NAME, :new.DEPARTMENT_NAME);
end;
...
-- Question 3
create or replace procedure execute_delete_by_id_sql_statement(p_delete_by_id_sql_statement in varchar2,
                                                               p_department_id in DEPARTMENTS.DEPARTMENT_ID%type) is
begin
...
create or replace procedure update_department_by_manager_id(old_Manager_ID in DEPARTMENTS.MANAGER_ID%type,
                                                            new_Manager_ID in DEPARTMENTS.MANAGER_ID%type) is
...
begin
    ...
    returning * bulk collect into v_departments;
...
declare
    cursor departments_curs (p_manager_id DEPARTMENTS.MANAGER_ID%type) is select *
                                                                          from DEPARTMENTS
                                                                          where MANAGER_ID = p_manager_id;
begin
    DBMS_OUTPUT.PUT_LINE('DEPARTMENT_ID ||  DEPARTMENT_NAME ||   MANAGER_ID ||  LOCATION_ID');
    for department_rec in departments_curs(149)
        loop
            DBMS_OUTPUT.PUT_LINE(department_rec.DEPARTMENT_ID || '  ||  ' || department_rec.DEPARTMENT_NAME ||
                                 '    ||  ' || department_rec.MANAGER_ID || ' ||  ' || department_rec.LOCATION_ID);
        end loop;
end;
...
BEGIN
    ...
    FOR I IN FIRST_NAME_VARIABLE.FIRST..FIRST_NAME_VARIABLE.LAST
        LOOP
...
CREATE OR REPLACE PACKAGE BODY TEST_PACKAGE IS
    PROCEDURE UPDATE_MANAGER(OLD_MANAGER_ID NUMBER, NEW_MANAGER_ID NUMBER,
                             DEPARTMENT_ROWS IN OUT DEPARTMENT_ROW_TYPE) IS
    BEGIN
...
BEGIN
    ...
    FOR I IN DEPARTMENT_ROW_TYPE_VARIABLE.FIRST..DEPARTMENT_ROW_TYPE_VARIABLE.LAST
        LOOP
            DBMS_OUTPUT.PUT_LINE(DEPARTMENT_ROW_TYPE_VARIABLE(I).DEPARTMENT_NAME ||
                                 DEPARTMENT_ROW_TYPE_VARIABLE(I).DEPARTMENT_ID);
        END LOOP;
END;
...
CREATE OR REPLACE TRIGGER DEPT_TRIGGER
    FOR UPDATE OF department_name
    ON departments COMPOUND TRIGGER
...
DECLARE
    CURSOR cur_dept_upd IS
        ...
        WHERE department_name LIKE 'A%'
            FOR UPDATE;
BEGIN
    FOR upd_dept_rec IN cur_dept_upd
        LOOP
            UPDATE departments

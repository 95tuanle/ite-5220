...
BEGIN
    FOR v_country_rec IN (SELECT COUNTRY_NAME, CURRENCY_CODE FROM WF_COUNTRIES NATURAL JOIN WF_CURRENCIES WHERE CURRENCY_NAME LIKE '%dollar%') LOOP
...
DECLARE
    CURSOR countries_cur IS
        SELECT COUNTRY_NAME, COUNTRY_ID, REGION_NAME FROM COUNTRIES NATURAL JOIN REGIONS WHERE REGION_NAME = 'Caribbean';
BEGIN
    FOR v_country_rec IN countries_cur LOOP
        DBMS_OUTPUT.PUT_LINE(v_country_rec.COUNTRY_NAME || ' ' || v_country_rec.COUNTRY_ID || ' ' || v_country_rec.REGION_NAME);
    END LOOP;
END;
...
DECLARE
    ...
    FROM WF_COUNTRIES NATURAL JOIN WF_WORLD_REGIONS
    WHERE REGION_NAME LIKE '%Europe%';
BEGIN
    FOR country_rec  IN countries_curs LOOP
        DBMS_OUTPUT.PUT_LINE(country_rec.COUNTRY_NAME || ' ' || country_rec.COUNTRY_ID || ' ' || country_rec.REGION_NAME);
    END LOOP;
END;
...
BEGIN
    ...
    FOR employee_rec IN (SELECT FIRST_NAME, LAST_NAME FROM EMPLOYEES) LOOP

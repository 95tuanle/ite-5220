...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(v_dept_rec.v_dept_id || ' ' || v_dept_rec.v_dept_name || ' ' || v_dept_rec.v_mgr_id || ' ' ||
                         v_dept_rec.v_loc_id);
EXCEPTION
...
BEGIN
    FOR country_name_rec IN (SELECT COUNTRY_ID, COUNTRY_NAME FROM COUNTRIES WHERE REGION_ID = 5 ORDER BY COUNTRY_ID)
        LOOP
            v_country_name(country_name_rec.COUNTRY_ID) := country_name_rec.COUNTRY_NAME;
        END LOOP;

    FOR i IN v_country_name.FIRST..v_country_name.LAST
        LOOP
...
DECLARE
    CURSOR emp_cur IS SELECT EMPLOYEE_ID, LAST_NAME, JOB_ID, SALARY
                      FROM EMPLOYEES
    ...
    FOR i IN v_emp.FIRST..v_emp.LAST
        LOOP
            IF v_emp.EXISTS(i) THEN
                DBMS_OUTPUT.PUT_LINE(i || ' ' || v_emp(i).LAST_NAME || ' ' || v_emp(i).JOB_ID || ' ' ||
                                     v_emp(i).SALARY);
            END IF;
        END LOOP;
END;

...
DECLARE
    CURSOR currencies_cur IS
        SELECT CURRENCY_CODE, CURRENCY_NAME FROM CURRENCIES ORDER BY CURRENCY_NAME ASC;
...
DECLARE
    CURSOR regions_curr IS
        SELECT REGION_NAME, COUNT(*) FROM REGIONS NATURAL JOIN COUNTRIES
        GROUP BY REGION_NAME HAVING COUNT(*) >= 10 ORDER BY REGION_NAME ASC;
...
BEGIN
    FOR countries_rec IN (SELECT country_name, national_holiday_name, national_holiday_date FROM countries WHERE region_id = 5) LOOP
        DBMS_OUTPUT.PUT_LINE ('Country: ' || countries_rec.country_name || ' National holiday: '||
                              countries_rec.national_holiday_name || ', held on: '|| countries_rec.national_holiday_date);
    END LOOP;
END;
...
DECLARE
    CURSOR countries_cur IS
        SELECT COUNTRY_NAME, HIGHEST_ELEVATION, CLIMATE FROM COUNTRIES WHERE HIGHEST_ELEVATION > 8000;
BEGIN
...
DECLARE
    CURSOR countries_cur IS
        SELECT COUNTRY_NAME, COUNT(*) as count FROM COUNTRIES NATURAL JOIN SPOKEN_LANGUAGES
...
DECLARE
    CURSOR countries_cur (p_region_id NUMBER, p_area NUMBER) IS
        SELECT COUNTRY_NAME, AREA FROM COUNTRIES WHERE REGION_ID = p_region_id AND AREA > p_area;
BEGIN
    FOR v_countries_rec IN countries_cur(5, 1000000) LOOP
        DBMS_OUTPUT.PUT_LINE(v_countries_rec.COUNTRY_NAME || ' ' || v_countries_rec.AREA);
    END LOOP;
END;
...
declare
    cursor employees_curr (p_salary EMPLOYEES.SALARY%type) is
        select * from EMPLOYEES where SALARY < p_salary for update nowait;
begin
    for v_employees_record in employees_curr(5000) loop
        insert into proposed_raises values (sysdate, null, v_employees_record.EMPLOYEE_ID, v_employees_record.DEPARTMENT_ID,
...
declare
    cursor proposed_raises_cur is select * from proposed_raises where department_id = 50 for update nowait;
begin
    for v_proposed_raises_rec in proposed_raises_cur loop
...
declare
    cursor employees_curr is
        select * from upd_emps for update nowait;
begin
...
DECLARE
    ...
    CURSOR employees_cur (p_department_id NUMBER) IS
        ...
        WHERE department_id = p_department_id ORDER BY last_name;
BEGIN
    FOR v_departments_rec IN departments_cur LOOP
        DBMS_OUTPUT.PUT_LINE(v_departments_rec.DEPARTMENT_ID || ' ' || v_departments_rec.DEPARTMENT_NAME);
        DBMS_OUTPUT.PUT_LINE('---------');
        FOR v_employees_rec IN employees_cur(v_departments_rec.DEPARTMENT_ID) LOOP
            DBMS_OUTPUT.PUT_LINE(v_employees_rec.FIRST_NAME || ' ' || v_employees_rec.LAST_NAME || ' ' || v_employees_rec.salary);
        END LOOP;
        DBMS_OUTPUT.PUT_LINE(' ');
    END LOOP;
END;
...
DECLARE
    CURSOR regions_cur IS
        SELECT REGION_ID, REGION_NAME FROM REGIONS WHERE REGION_NAME like '%America%'
        ORDER BY region_name;
    CURSOR countries_cur (p_region_id NUMBER) IS
        SELECT COUNTRY_NAME, AREA, POPULATION, COUNTRY_ID FROM COUNTRIES
        WHERE REGION_ID = p_region_id ORDER BY COUNTRY_NAME;
    CURSOR languages_cur (p_country_id NUMBER) IS
        SELECT LANGUAGE_NAME FROM LANGUAGES NATURAL JOIN SPOKEN_LANGUAGES
        WHERE COUNTRY_ID = p_country_id ORDER BY LANGUAGE_NAME;
BEGIN
    FOR v_regions_rec IN regions_cur LOOP
        DBMS_OUTPUT.PUT_LINE(v_regions_rec.REGION_ID || ' ' || v_regions_rec.REGION_NAME);
        DBMS_OUTPUT.PUT_LINE('---------');
        FOR v_countries_rec IN countries_cur(v_regions_rec.REGION_ID) LOOP
            DBMS_OUTPUT.PUT_LINE(v_countries_rec.COUNTRY_NAME || ' ' || v_countries_rec.AREA || ' ' || v_countries_rec.POPULATION);
            FOR v_languages_cur IN languages_cur(v_countries_rec.COUNTRY_ID) LOOP

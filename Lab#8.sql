...
DECLARE
    ...
    CURSOR regions_curs IS SELECT * FROM regions WHERE region_id < 20
    ORDER BY region_id;
...
CREATE PROCEDURE pay_raise
    (p_empid employees.employee_id%TYPE, p_percent_increase NUMBER) IS
BEGIN
    UPDATE employees
...
select SALARY from employees_dup;
...
select SALARY, DEPARTMENT_ID from employees_dup;
...
create or replace procedure get_number_of_countries (p_region_id in COUNTRIES.REGION_ID%type, p_highest_elevation in COUNTRIES.HIGHEST_ELEVATION%type) is
    v_number_of_countries number;
begin
    select count(*) into v_number_of_countries from COUNTRIES where REGION_ID = p_region_id and HIGHEST_ELEVATION > p_highest_elevation;
...
begin
    select COUNTRY_NAME, POPULATION into p_country_name, p_country_population from COUNTRIES where COUNTRY_ID = p_country_id;
    exception
    when NO_DATA_FOUND then
...
select COUNTRY_ID from COUNTRIES where lower(COUNTRY_NAME) like '%viet%';

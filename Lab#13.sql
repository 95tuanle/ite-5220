...
NULL;
END;
PROCEDURE do_nothing2 (tab IN OUT NOCOPY EmpTabTyp) IS BEGIN
NULL;
END;
END nocopy_test;
...
begin
    ...
    forall i in v_emp_id_tab.FIRST..v_emp_id_tab.LAST delete from emp_temp where EMPLOYEE_ID = v_emp_id_tab(i)
                                                                   returning LAST_NAME bulk collect into v_emp_name_tab;
    DBMS_OUTPUT.PUT_LINE('Deleted ' ||  SQL%ROWCOUNT || ' rows:');
    for i in v_emp_id_tab.FIRST..v_emp_id_tab.LAST loop
...
VALUES (p_empid, p_fname, p_lname, p_email,p_hdate, p_job, p_dept_id); ELSE
DBMS_OUTPUT.PUT_LINE('Invalid department number given, please try again.'); END IF;
END;
...
select OBJECT_NAME, OBJECT_TYPE, STATUS from USER_OBJECTS where OBJECT_NAME in ('EMPLOYEES', 'NEW_EMP', 'CHECK_DEPT');



describe USER_DEPENDENCIES;

select NAME, TYPE from USER_DEPENDENCIES where REFERENCED_NAME = 'EMPLOYEES' order by TYPE;

...
create or replace package body EMP_PKG is
    ...
    function CHECK_FNAME (p_first_name in EMPLOYEES.FIRST_NAME%type) return boolean is
        cursor emp_curs is select FIRST_NAME from EMPLOYEES;
    begin
        for emp_rec in emp_curs loop
...
create table audit_employees (action varchar2(50), username varchar2(30) default user, timestamp timestamp default systimestamp, employee_id number);

create or replace trigger Audit_Employees after insert or delete or update on EMPLOYEES for each row
begin
    if inserting then
...
insert into EMPLOYEES (EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, JOB_ID, SALARY, COMMISSION_PCT, MANAGER_ID, DEPARTMENT_ID) values (1001,'Steven','King','SKIN','515.123.4567',TO_DATE('1987-06-17','yyyy-mm-dd'),'AD_PRES',24000,null,null,90);

update EMPLOYEES set EMPLOYEE_ID = 1003 where EMPLOYEE_ID = 1001;

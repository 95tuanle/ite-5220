create or replace procedure display_department_id(p_table_name varchar2,
                                                  p_department_id out EMPLOYEES.DEPARTMENT_ID%type) is
begin
...
CREATE TABLE copy_countries AS
SELECT *
FROM countries;
...
AS
SELECT department_id dept_id, count(*) count_emps
FROM employees
...
begin
    ...
    begin
        ...
        values ('Updated, old salary was ' || :old.SALARY || ' new salary is ' || :new.SALARY, :new.EMPLOYEE_ID);
    end after each row;
    end;
...
select TRIGGER_BODY
from USER_TRIGGERS
where TRIGGER_NAME = upper('max_salary_trigger');
...
create or replace procedure reenable_triggers_proc is
    cursor triggers_curs is select TRIGGER_NAME
                            from USER_TRIGGERS
                            where STATUS = 'DISABLED';
begin
    for trigger in triggers_curs
        loop

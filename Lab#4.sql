...
BEGIN
    ...
    INTO v_ind_date, v_natl_holiday FROM countries
...
BEGIN
    SELECT COUNT(*) INTO v_number_of_countries FROM COUNTRIES NATURAL JOIN CURRENCIES WHERE CURRENCY_NAME = v_currency;
    CASE
        WHEN v_number_of_countries > 20 THEN DBMS_OUTPUT.PUT_LINE('More than 20 countries');
        WHEN v_number_of_countries <= 20 AND v_number_of_countries >= 10 THEN DBMS_OUTPUT.PUT_LINE('Between 10 and 20 countries');
...
SELECT stars FROM new_emps WHERE employee_id = 142;
...
BEGIN
    <<OUTER_LOOP>>
    FOR v_first_number IN 60..65 LOOP
        <<INNER_LOOP>>
        FOR v_second_number IN 100..110 LOOP

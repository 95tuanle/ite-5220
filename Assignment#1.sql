...
AS (SELECT employee_id, first_name || ' ' || last_name, salary, job_id
    FROM employees);
...
DECLARE
    CURSOR emp_curs IS
        ...
        FROM emp
                 JOIN jobs ON emp.job = jobs.JOB_ID
        WHERE JOB_TITLE != 'President'
        ORDER BY empno FOR UPDATE NOWAIT;
...
DECLARE
    CURSOR languages_curs (p_country_name COUNTRIES.COUNTRY_NAME%TYPE) IS
        ...
        WHERE COUNTRY_NAME LIKE '%' || p_country_name || '%'
          AND (OFFICIAL = 'Y' OR OFFICIAL = 'QY');
BEGIN
    FOR v_language_rec IN languages_curs('Viet')
        LOOP
...
DECLARE
    CURSOR employees_curs IS
        SELECT FIRST_NAME, TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE) / 12) as YEARS
        FROM EMPLOYEES
                 JOIN DEPARTMENTS
                      ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID
        WHERE DEPARTMENT_NAME LIKE '%Sales%';
BEGIN
    FOR v_employee_rec IN employees_curs
        LOOP
            DBMS_OUTPUT.PUT_LINE(v_employee_rec.FIRST_NAME || ' has worked in the company for ' ||
                                 v_employee_rec.YEARS || ' years.');
        END LOOP;
END;
...
DECLARE
    CURSOR orders_curs (p_order_number F_ORDERS.ORDER_NUMBER%TYPE) IS
        SELECT DISTINCT ORDER_NUMBER, SUM(PRICE * QUANTITY) as TOTAL
        FROM F_ORDERS
                 NATURAL JOIN F_ORDER_LINES
                 NATURAL JOIN F_FOOD_ITEMS
        WHERE ORDER_NUMBER = p_order_number
        GROUP BY ORDER_NUMBER;
    v_trigger NUMBER;
BEGIN
    FOR v_order_rec IN orders_curs(5800)
        LOOP
...
DECLARE
    ...
    CURSOR c1 IS
        SELECT FOOD_ITEM_NUMBER, QUANTITY
        FROM f_order_lines
        WHERE ORDER_NUMBER = in_order_num;
BEGIN
    OPEN c1;
    LOOP
    ...
    IF (totalPrice > 100) THEN
        DBMS_OUTPUT.PUT_LINE('This order ' || in_order_num || ' with total price ' || totalPrice ||
...
EXCEPTION
    WHEN below_limit THEN
        DBMS_OUTPUT.PUT_LINE('This order ' || in_order_num || ' with total ' || totalPrice ||
                             ' is below discount limit.');
END;
...
CREATE OR REPLACE PROCEDURE get_country_name_proc(p_country_id IN countries.country_id%TYPE,
...
END;
SELECT get_country_name_proc(country_id)
FROM countries;
SELECT get_country_name_func(country_id)
FROM countries;
...
create or replace procedure insert_emp(p_employee_id in EMPLOYEES.EMPLOYEE_ID%type,
                                       p_last_name in EMPLOYEES.LAST_NAME%type, p_salary in EMPLOYEES.SALARY%type,
...
create or replace procedure ins_new_dept(p_department_id in NEW_DEPT_TAB.DEPARTMENT_ID%type,
                                         p_department_name in NEW_DEPT_TAB.DEPARTMENT_NAME%type,
                                         p_manager_id in NEW_DEPT_TAB.MANAGER_ID%type,
                                         p_location_id in NEW_DEPT_TAB.LOCATION_ID%type) is
begin
